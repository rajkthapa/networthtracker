    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="stat-card border-l-4 border-success-500/50">
          ...
          <p className="text-[10px] text-th-faint mt-0.5">60% of current expenses</p>
          <div className="w-full h-2 bg-[var(--bg-hover)] rounded-full mt-2 overflow-hidden">
            <div className="h-full bg-success-500 rounded-full" style={{ width: `${Math.min(leanFireProgress, 100)}%` }} />
          </div>
          <p className="text-xs text-th-faint mt-1">{leanFireProgress.toFixed(1)}% complete</p>
        </div>

        <div className="stat-card border-l-4 border-primary-500/50">
          <div className="flex items-center gap-2 mb-2">
            <Flame className="w-5 h-5 text-primary-500" />
            <p className="text-sm font-semibold text-th-heading">Regular FIRE</p>
          </div>
          <p className="text-xl font-bold text-th-heading num">{formatCurrency(fireNumber, true)}</p>
          <p className="text-[10px] text-th-faint mt-0.5">100% of current expenses</p>
          <div className="w-full h-2 bg-[var(--bg-hover)] rounded-full mt-2 overflow-hidden">
            <div className="h-full bg-primary-500 rounded-full" style={{ width: `${Math.min(fireProgress, 100)}%` }} />
          </div>
          <p className="text-xs text-th-faint mt-1">{fireProgress.toFixed(1)}% complete</p>
        </div>

        <div className="stat-card border-l-4 border-grape-500/50">
          <div className="flex items-center gap-2 mb-2">
            <Zap className="w-5 h-5 text-grape-500" />
            <p className="text-sm font-semibold text-th-heading">Coast FIRE</p>
          </div>
          <p className="text-xl font-bold text-th-heading num">{formatCurrency(coastFireNumber, true)}</p>
          <p className="text-[10px] text-th-faint mt-0.5">Stop saving, retire at 65</p>
          <div className="w-full h-2 bg-[var(--bg-hover)] rounded-full mt-2 overflow-hidden">
            <div className="h-full bg-grape-500 rounded-full" style={{ width: `${Math.min(coastFireProgress, 100)}%` }} />
          </div>
          <p className="text-xs text-th-faint mt-1">{coastFireProgress.toFixed(1)}% complete</p>
        </div>

        <div className="stat-card border-l-4 border-accent-500">
          <div className="flex items-center gap-2 mb-2">
            <Award className="w-5 h-5 text-accent-500" />
            <p className="text-sm font-semibold text-th-heading">Fat FIRE</p>
          </div>
          <p className="text-xl font-bold text-th-heading num">{formatCurrency(fatFireNumber, true)}</p>
          <p className="text-[10px] text-th-faint mt-0.5">150% of current expenses</p>
          <div className="w-full h-2 bg-[var(--bg-hover)] rounded-full mt-2 overflow-hidden">
            <div className="h-full bg-accent-500 rounded-full" style={{ width: `${Math.min(fatFireProgress, 100)}%` }} />
          </div>
          <p className="text-xs text-th-faint mt-1">{fatFireProgress.toFixed(1)}% complete</p>
        </div>
      </div>
      ...
      <div className="chart-container">
        ...
        <ResponsiveContainer width="100%" height={400}>
          <AreaChart data={projectionData}>
            ...
            <XAxis dataKey="year" stroke="var(--chart-axis)" fontSize={11} tickLine={false} />
            ...
            <Area type="monotone" dataKey="leanFire" stroke="#10b981" strokeDasharray="5 5" fill="none" strokeWidth={1.5} name="leanFire" />
            <Area type="monotone" dataKey="fireTarget" stroke="#ef4444" strokeDasharray="5 5" fill="none" strokeWidth={2} name="fireTarget" />
            <Area type="monotone" dataKey="netWorth" stroke="#14b8a6" fill="url(#projGrad)" strokeWidth={3} name="netWorth" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
        ...
        <div className="chart-container bg-gradient-to-br from-warning-500/10 to-danger-500/10">
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-3 p-3 bg-th-card/80 rounded-xl">
              ...
              <div>
                <p className="text-sm font-semibold text-th-heading">Increase savings rate to 80%</p>
                <p className="text-xs text-th-muted">Could reduce your FIRE timeline by ~3 years</p>
              </div>
            </div>
            <div className="flex items-start gap-3 p-3 bg-th-card/80 rounded-xl">
              <span className="text-xl">📈</span>
              <div>
                <p className="text-sm font-semibold text-th-heading">Max out tax-advantaged accounts</p>
                <p className="text-xs text-th-muted">401(k) $23,500 + Roth IRA $7,000 + HSA $4,150 = $34,650/yr</p>
              </div>
            </div>
            <div className="flex items-start gap-3 p-3 bg-th-card/80 rounded-xl">
              <span className="text-xl">💡</span>
              <div>
                <p className="text-sm font-semibold text-th-heading">Grow dividend income</p>
                <p className="text-xs text-th-muted">Your passive income covers {expenseCoverage.toFixed(0)}% of expenses — aim for 100%</p>
              </div>
            </div>
            <div className="flex items-start gap-3 p-3 bg-th-card/80 rounded-xl">
              <span className="text-xl">🏠</span>
              <div>
                <p className="text-sm font-semibold text-th-heading">Eliminate high-interest debt first</p>
                <p className="text-xs text-th-muted">Credit cards at 22-25% APR cost more than investments earn</p>
              </div>
            </div>
            <div className="flex items-start gap-3 p-3 bg-th-card/80 rounded-xl">
              <span className="text-xl">📊</span>
