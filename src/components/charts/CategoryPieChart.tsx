      ...
      <div className="chart-container">
        ...
        <div className="flex flex-col items-center justify-center py-16 gap-3">
          <div className="w-14 h-14 rounded-2xl bg-surface-50 flex items-center justify-center">
            <PieIcon className="w-7 h-7 text-surface-300" />
          </div>
          <p className="text-sm font-medium text-surface-600">No {type} data this month</p>
          <p className="text-xs text-surface-400 text-center max-w-[200px]">Add {type === 'income' ? 'income' : 'expenses'} to see your category breakdown</p>
        </div>
      </div>
    );
  }
                  ...
                  return (
                    <div className="bg-white border border-surface-200 rounded-xl px-4 py-3 shadow-lg">
                      <div className="flex items-center gap-2 mb-1">
                        <span>{d.icon}</span>
                        <span className="text-sm font-semibold text-surface-800">{d.name}</span>
                      </div>
                      <p className="text-sm font-bold text-surface-900 num">{formatCurrency(d.amount)}</p>
                      <p className="text-xs text-surface-500">{d.percentage.toFixed(1)}% of total</p>
          ...
          <div className="w-full lg:w-auto space-y-1 max-h-60 overflow-y-auto scrollbar-hide">
            {data.map((item) => (
              <button
                ...
                className="flex items-center gap-2 text-sm whitespace-nowrap w-full text-left px-2 py-1.5 rounded-lg hover:bg-surface-50 transition-colors group"
              >
                <div className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: item.color }} />
                <span className="text-surface-600 text-xs">{item.icon}</span>
                <span className="text-surface-700 font-medium text-xs group-hover:text-primary-600 transition-colors">{item.name}</span>
                <span className="text-surface-800 text-xs ml-auto pl-3 font-semibold num">{formatCurrency(item.amount)}</span>
